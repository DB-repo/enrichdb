...
gender_et_new = pickle.load(open('/home/ubuntu/supermadlib/SuperMADLib/backend/load/multi_pie_clfs/gender_multipie_et_calibrated_new.p', 'rb'))

gender_rf_new = pickle.load(open('/home/ubuntu/supermadlib/SuperMADLib/backend/load/multi_pie_clfs/gender_multipie_rf_calibrated.p', 'rb'))


gender_gnb_new = pickle.load(open('/home/ubuntu/supermadlib/SuperMADLib/backend/load/multi_pie_clfs/gender_multipie_gnb_calibrated_new.p', 'rb'))
gender_dt = joblib.load(open('/home/ubuntu/supermadlib/SuperMADLib/backend/load/multi_pie_clfs/gender_multipie_dt_calibrated.p', 'rb')) # 5K features
gender_mlp_new =  pickle.load(open('/home/ubuntu/supermadlib/SuperMADLib/backend/load/multi_pie_clfs/gender_multipie_mlp_calibrated_new.p', 'rb'))
...
def exec_gender_gnb(rl):
        cProb = gender_gnb_new.predict_proba(rl)
        return cProb[0]

def exec_gender_dt(rl):
        cProb = gender_dt.predict_proba(rl)
        return cProb[0]
def exec_gender_et_new(rl):
        cProb = gender_et_new.predict_proba(rl)
        return cProb[0]
def exec_gender_rf_new(rl):
        cProb = gender_rf_new.predict_proba(rl)
        return cProb[0]

def exec_gender_mlp(rl):
        cProb = gender_mlp_new.predict_proba(rl)
        return cProb[0]
...
def _response(input_features):
    ...
    if fcname == 'tweet_sentiment_all':
        ...
        #plpy.notice(proba)
        response = 0
    elif fcname == 'image_gender_all':
        bitmap = [1,1,1,1]
        prob2DArray =[]
        features=input_features

        proba = exec_gender_gnb([features[:10000]])
        prob2DArray.append(proba)

        proba = exec_gender_dt([features[:10000]])
        prob2DArray.append(proba)

        proba = exec_gender_rf_new([features[:10000]])
        prob2DArray.append(proba)

        proba = exec_gender_mlp([features[:10000]])
        prob2DArray.append(proba)
...
if __name__ == "__main__":
    ...
    start_id = 2000
    end_id = 3000
    _select_sql = "(select i1.id,i1.image_blob,i1.feature, i2.gender from images i1, images_full i2 where i1.id = i2.id and i1.id >" + str(start_id)+ " and i1.id <" + str(end_id) +  " and i2.id >" + str(start_id) +" and i2.id  <" + str(end_id) + ") as my_table"
    df_select = spark.read.jdbc(url="jdbc:postgresql://localhost:5432/test",table=_select_sql,properties=properties)
    df_select.show()
    truth_list = df_select.select('gender').rdd.flatMap(lambda x: x).collect()
    all_f1 = []
    truth = 0
...
#df4= df_select.withColumn('id',output_udf_float('feature')).show()
        ...
        for j in range(len(pred_list)):
            if pred_list[j] == 0:
                count+=1
            if pred_list[j] == truth_list[j + i*epoch_size] and pred_list[j] == 0:
                correct+=1
